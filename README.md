...
| 📚 **实战知识库** | 1167 条蒸馏知识单元（48 分类 + 领域素材层 + 法定公文成稿范例层 + 报告蒸馏层 + 八书蒸馏层 + 七博主层（v0.3 凌云+惠水 → v0.5 摆渡+捉刀人 → v0.8 改稿室 → v0.9 大雨 → v0.11 文兄 → v0.18 掌上课堂+何剑文+心法））：标题公式、即兴发言框架、汇报口诀、金句素材、词句升级、改稿对照、思路框架、数字框架库、领域素材、法定公文成稿范例 | references/corpus-lingyun.md |
...
> 视频类内容经授权渠道下载 → 语音转写 → 逐条蒸馏 → 结构化整理；书籍类内容经微信读书导出蒸馏。结构化数据集（1231 条 jsonl）另存。
...
│   ├── corpus-lingyun.md   # 蒸馏知识库索引页（索引+17 分层文件：层索引+检索规则，条目正文在 corpus/）
│   └── corpus/             # 分层知识库 17 文件（主层/文种规范/核稿病例/捉刀人/公文摆渡/报告蒸馏/手记2/手记1/付传/掌上课堂/何剑文/心法/改稿室/大雨/文兄/领域素材/法定公文成稿范例）+ INDEX.md 知识分类索引（自动生成）
├── scripts/
...
<details>
<summary><b>当前 v0.18</b>（2026-09-16）· v0.1 → v0.18 完整明细点击展开 · 逐版本知识增量见 <a href="CHANGELOG.md">CHANGELOG.md</a></summary>
